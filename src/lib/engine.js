...
// define engine
export const SyntaxeEngine = class {
	#holder;
	...
	// filter operations
	async #filterOperations(schema) {
		try {
			// initialize operation maps
			this.#holder.propertyOps = new Map();
			this.#holder.objectOps = new Map();
			this.#holder.rootOp = null;
			this.#holder.context = 'json';

			let operationsString;

			// check schema context
			String(schema).trim().startsWith('[')
				? ((this.#holder.context = 'root') && (schema = `{ rootPropertyIdentifier }${schema}`))
				: true;
			...
			const filtered = String(schema)
		        ...
		        let indexOfDelimiter = _.split('').indexOf('['),
	      		...
	      		this.#holder.propertyOps.set(name, {
		        ...
		        let indexOfDelimiter = _.split('').indexOf('['),
	      		...
	      		this.#holder.condition = operationsString.match(/"cond:/) && operationsString.match(/"cond:\\"or\\"/) ? 'or' : 'and';
	      		this.#holder.mode = operationsString.match(/"mode:/) && operationsString.match(/"mode:\\"or\\"/) ? 'or' : 'and';

	      		// remove mode and condition
	      		operations = operations.filter(x => !x.match(/mode:/) && !x.match(/cond:/));

	      		// get root operations
		        this.#holder.rootOp = operations;
		        return `}`;
			    })
		        ...
		        this.#holder.objectOps.set(name, {
		        	operation: operations,
		        	mode: propertyMode,
		        	condition: propertyCondition
		        });
	...
	// filter the schema
	async filterSchema(schema) {
		try {
			// filter operations
			let filtered = await this.#filterOperations(schema);
	...
	async walkThroughHandler({ data, status, schema }) {
		// check if no schema
		...
		try {
			...
			let result = (this.#holder.context == 'json') 
				? (await this.#schemaWalkThrough({ schema, subject })).result
				: subject;

			// check if root operations available
			if (this.#holder.rootOp) {
				// pass new random name
				const name = `*instr-p:id_${randomKey()}`;

	  		// pass property name and operations into holder
	  		this.#holder.propertyOps.set(name, {
	  			property: this.#holder.rootKey,
	  			operation: this.#holder.rootOp,
	  			condition: this.#holder.condition
	  		});

	  		// compute status
				let operationStatus = await this.#schemaWalkThrough({ schema: { [name]: 1 }, subject: { [this.#holder.rootKey]: result } })
				result = operationStatus.schemaPass 
					? operationStatus.result[this.#holder.rootKey]
					: operationStatus.result[this.#holder.rootKey] instanceof Array ? [] : Object.create(null);
			}
			return result;
		} catch(err) {
	...
	async #schemaWalkThrough({ schema, subject, mode }) {
		...
		if (Array.isArray(subject)) {
			...
			for (let item of subject) {
				let line = await this.#schemaWalkThrough({ schema: schema, subject: item, mode });
			...
			for (let key of properties) {
				...
				if (this.#holder.propertyOps.has(key)) {
					// get property info
					propertyInfo = this.#holder.propertyOps.get(key);
						...
						for (let ndex = 0; ndex < propertyInfo.operation.length; ndex++) {
							...
							switch (ls) {
								...
								case 'tlt': // check if time is less than
									...
									operationObject.valueDateTime = dateify(...this.#holder.defaultDate, ...operationObject.valueTimeParts);
									operationObject.filteredDateTime = dateify(...this.#holder.defaultDate, ...operationObject.timeParts);
										...
										operationObject.hasMeridiem 
											&& (operationObject.timeInfo.hour += ((operationObject.timeInfo.meridiem == 'pm') && (operationObject.timeInfo.hour != 12)) 
													? 12 : ((((operationObject.timeInfo.meridiem == 'am') && (operationObject.timeInfo.hour != 12)) || (operationObject.timeInfo.meridiem == 'am')) 
														? 0 : -12));
										operationObject.timeParts[0] = operationObject.timeInfo.hour;
										return dateify(...this.#holder.defaultDate, ...operationObject.timeParts);
									});
				...
				// check if property exists in normal schema
					...
					if (schema[key] === 1) {
						...
						if (this.#holder.objectOps.has(properties[keyCounter + 1])) {
							let objectPropertyInfoKey = properties[keyCounter + 1],
								prop = key;
								
							// extract object info
							objectPropertyInfo = this.#holder.objectOps.get(objectPropertyInfoKey);
				    		...
				    		// pass property name and operations into holder
				    		this.#holder.propertyOps.set(currentKeyAsNewPropertyName, {
				    			property: prop,
						...
						if (!keyOmissionStatus || (keyOmissionStatus && objectHasOperationsStatus))
							result[objectPropertyInfo?.currentKeyAlias || key] = (await this.#schemaWalkThrough({ schema: schema[key] ?? schema[keyAsIs], subject: subject[key], mode: currentKeyMode })).result;

						// check if key has operations waiting in next key
						if (objectHasOperationsStatus) {
							// compute status
							let operationStatus = await this.#schemaWalkThrough({ schema: { [currentKeyAsNewPropertyName]: 1  }, subject: { [key]: result[objectPropertyInfo?.currentKeyAlias || key] } })
							if (!keyOmissionStatus) {
			...
			// compute pass
			schemaPass = (this.#holder.propertyOps.size < 1) 
				? true 
				: (schemaPassSet.size > 1) 
					? ((mode ?? this.#holder.mode) == 'or' ? true : false) 
